...
def remove_user(
    ...
    print(f"REMOVE REQUEST — clientid: '{clientid}' | userid: '{userid}'")
    ...
    if not os.path.exists(index_path) or not os.path.exists(mapping_path):
        return {"success": False, "message": "No face database found for this client"}
    ...
    for vector_id_str, user_data in current_mapping.items():
        ...
        if str(stored_userid) == str(userid):
            print("EXACT MATCH FOUND:", vector_id_str)
            ids_to_remove.add(int(vector_id_str))
            continue

        if (
            stored_userid is not None and
            len(str(stored_userid)) == len(str(userid)) == 18 and
            str(stored_userid)[:14] == str(userid)[:14]
        ):
            print(f"PRECISION MATCH: stored={stored_userid} received={userid}")
            ids_to_remove.add(int(vector_id_str))
    ...
    ids_to_keep = [
        int(vid) for vid in current_mapping.keys()
